...
def build_session(moves):
    ...
    for step in range(int(len(moves) / 2)):
        ...
        # game.show()
        if side == 1:
            player1_state_actions.append([state, action, 0])
        else:
            player2_state_actions.append([state * -1, action.reverse(), 0])
        side *= -1
...
def replay_game(moves):
    ...
    side = 1
    for step in range(int(len(moves) / 2)):
        game.update(wuziqi.WuziqiAction(int(moves[step * 2], 16) - 1, int(moves[step * 2 + 1], 16) - 1, side))
        game.show()
        side *= -1
