...
class ValueRestNet(object):
    def __init__(self, name, board_size, learning_rate, lbd):
        ...
        layer1 = self.conv_bn_relu(1, input_layer, 64, [2, 2], [1, 1], 'valid')
        layer2 = self.conv_bn_relu(2, layer1, 64, [2, 2], [1, 1], 'same')
        layer3 = self.conv_bn_relu(3, layer2, 64, [2, 2], [1, 1], 'same', False)
        rest3 = self.resAdd_relu(layer1, layer3, 3)
        layer4 = self.conv_bn_relu(4, rest3, 128, [2, 2], [2, 2], 'valid')
        layer5 = self.conv_bn_relu(5, layer4, 128, [2, 2], [1, 1], 'same')
        layer6 = self.conv_bn_relu(6, layer5, 128, [2, 2], [1, 1], 'same', False)
        rest6 = self.resAdd_relu(layer4, layer6, 6)
        layer7 = self.conv_bn_relu(7, rest6, 512, [2, 2], [2, 2], 'valid')
        layer8 = self.conv_bn_relu(8, layer7, 512, [2, 2], [1, 1], 'same')
        layer9 = self.conv_bn_relu(9, layer8, 512, [2, 2], [1, 1], 'same', False)
        rest9 = self.resAdd_relu(layer7, layer9, 9)
        pool9 = tf.layers.average_pooling2d(
            name=name + "value_net_pool9",
            inputs=rest9,
            pool_size=self.pool_size,
            strides=1)

        flat_size = 2048
        pool_flat = tf.reshape(pool9, [-1, flat_size], name=name + "value_net_pool_flat")
