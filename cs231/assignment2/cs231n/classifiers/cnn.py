...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    HH = int(1 + (input_dim[1] + 2 * pad - filter_size) / stride)/pool_size
    WW = int(1 + (input_dim[2] + 2 * pad - filter_size) / stride)/pool_size

    coutput_shape = (num_filters, HH, WW)

    self.params['W2'] = np.random.randn(np.prod(coutput_shape), hidden_dim) * weight_scale
  ...
  def loss(self, X, y=None):
    ...
    dout, dW3, grads['b3'] = affine_backward(dout, cache3)
    dout, dW2, grads['b2'] = affine_relu_backward(dout, cache2)
    dout, dW1, grads['b1'] = conv_relu_pool_backward(dout, cache1)
