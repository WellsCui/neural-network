...
def rnn_step_backward(dnext_h, cache):
  ...
  dx = dtanh.dot(Wx.transpose())
  dprev_h = dtanh.dot(Wh.transpose())
  dWx = x.transpose().dot(dtanh)
  dWh = prev_h.transpose().dot(dtanh)
...
def rnn_backward(dh, cache):
  ...
  for i in np.array(range(T))[::-1]:
    dnext_h = dh[:, i, :] + dpre_h
    dx_i, dpre_h, dWx_i, dWh_i, db_i = rnn_step_backward(dnext_h, cache[i])
    dx[:, i, :] = dx_i
    dWx += dWx_i
    dWh += dWh_i
    db += db_i
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  x, prev_h, prev_c, Wx, Wh, b, i, f, o, g, next_c_tanh = cache
  do = next_c_tanh * dnext_h * o * (1 - o)
...
def lstm_backward(dh, cache):
  ...
  for i in np.array(range(T))[::-1]:
    ...
    dx_i, dprev_h, dnext_c, dWx_i, dWh_i, db_i = lstm_step_backward(dnext_h, dnext_c, cache[i])
    dx[:, i, :] = dx_i
    dWx += dWx_i
    dWh += dWh_i
    db += db_i
